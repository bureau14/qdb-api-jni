...
/* static */ void qdb::jni::log::swap_callback()
{
    // There may be race conditions in the `local_callback_id` and us adding /
    // removing the lock.
    //
    // As it's unlikely that this specific function is a bottleneck, let's just
    // acquire a unique lock here.
...
void qdb::jni::log::_do_log(qdb_log_level_t log_level,
    ...
    std::string const & msg)
{
    qdb::jni::log::message_t x{log_level,
        ...
        static_cast<int>(pid), static_cast<int>(tid), msg};

    std::unique_lock guard(buffer_lock);
    buffer.push_back(x);
}

/* static */ void qdb::jni::log::_callback(qdb_log_level_t log_level,
    const unsigned long * date,
    unsigned long pid,
    unsigned long tid,
    ...
    size_t message_size)
{
    _do_log(log_level, date, pid, tid, std::string(message_buffer, message_size));
}

void qdb::jni::log::_current_date(unsigned long * date)
{
    time_t now      = time(0);
...
/* static */ void qdb::jni::log::flush(qdb::jni::env & env)
{
...
/* static */ void qdb::jni::log::_do_flush(qdb::jni::env & env)
{
    ...
    for (auto i = buffer.begin(); i != buffer.end(); ++i)
    {
        ...
        env.instance().CallStaticVoidMethod(*qdbLogger, *logID,
            ...
            m.timestamp.year, m.timestamp.mon, m.timestamp.day, m.timestamp.hour, m.timestamp.min,
            m.timestamp.sec,
