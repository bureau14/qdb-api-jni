    ...
    qdb::jni::env & env, jobject input)
{
    ...
    jni::guard::string_utf8 name = jni::string::from_field(env, input, name_field);
    qdb_ts_column_type_t type =
        _column_type_from_type_enum(env, jni::object::from_field<jobject>(env, input, type_field));

    return qdb_ts_column_info_ex_t{name.copy(), type,
        (type == qdb_ts_column_symbol
                ? jni::string::from_field(env, input, symbol_table_field).copy()
                : nullptr)};
}
    ...
    qdb::jni::env & env, jobject input)
{
    ...
    jni::guard::string_utf8 table  = jni::string::from_field(env, input, table_field);
    jni::guard::string_utf8 column = jni::string::from_field(env, input, column_field);

    return qdb_ts_batch_column_info_t{table.copy(), column.copy(),
...
/* static */ qdb_ts_column_info_t qdb::jni::adapt::column::to_qdb(
    qdb::jni::env & env, jobject input)
{
    qdb_ts_column_info_ex_t x = to_qdb<qdb_ts_column_info_ex_t>(env, input);
