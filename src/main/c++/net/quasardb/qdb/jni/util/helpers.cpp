...
setReferenceValue(qdb::jni::env &env, jobject reference, jobject value)
{
    ...
    if (!fid)
    {
        ...
        fid =
            env.instance().GetFieldID(thisClass, "value", "Ljava/lang/Object;");
    }
              ...
              jlong size)
{
    setReferenceValue(
        env, reference,
        ptr ? env.instance().NewDirectByteBuffer((void *)ptr, size) : NULL);
}
...
setInteger(qdb::jni::env &env, jobject reference, jint value)
{
    ...
    {
        constructorId =
            env.instance().GetMethodID(integerClass, "<init>", "(I)V");
    }
    setReferenceValue(
        env, reference,
        env.instance().NewObject(integerClass, constructorId, value));
}
...
setLong(qdb::jni::env &env, jobject reference, jlong value)
{
    ...
    setReferenceValue(
        env, reference,
        env.instance().NewObject(longClass, constructorId, value));
}

void
setStringArray(qdb::jni::env &env,
               jobject reference,
               const char **strings,
               size_t count)
{
    ...
    jobjectArray array =
        env.instance().NewObjectArray((jsize)count, stringClass, NULL);
