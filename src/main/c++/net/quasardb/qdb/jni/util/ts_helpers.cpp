                            ...
                            jobject value)
{
    ...
    jmethodID methodId =
        env.instance().GetMethodID(objectClass, "getInt64", "()J");
    assert(methodId != NULL);
    ...
    return jni::exception::throw_if_error(
        handle, qdb_ts_batch_row_set_int64(batchTable, index, v));
}
                             ...
                             jobject value)
{
    jclass objectClass = env.instance().GetObjectClass(value);
    jmethodID methodId =
        env.instance().GetMethodID(objectClass, "getDouble", "()D");
    assert(methodId != NULL);
    ...
    return jni::exception::throw_if_error(
        handle, qdb_ts_batch_row_set_double(batchTable, index, v));
}
                                ...
                                jobject value)
{
    ...
    jmethodID methodId = env.instance().GetMethodID(
        objectClass, "getTimestamp", "()Lnet/quasardb/qdb/ts/Timespec;");
    assert(methodId != NULL);
    ...
    jni::exception::throw_if_error(
        handle, qdb_ts_batch_row_set_timestamp(batchTable, index, &timestamp));
...
{
    jclass objectClass = env.instance().GetObjectClass(value);
    jmethodID methodId = env.instance().GetMethodID(objectClass, "getBlob",
                                                    "()Ljava/nio/ByteBuffer;");
    assert(methodId != NULL);
    ...
    qdb_size_t blob_size =
        (qdb_size_t)(env.instance().GetDirectBufferCapacity(blobValue));

    qdb::jni::log::trace("Batch writer setting blob at offset %d", index);

    jni::exception::throw_if_error(
        handle,
        qdb_ts_batch_row_set_blob(batchTable, index, blob_addr, blob_size));
                             ...
                             jobject value)
{
    ...
    jclass objectClass = env.instance().GetObjectClass(value);
    jmethodID methodId = env.instance().GetMethodID(objectClass, "getString",
                                                    "()Ljava/lang/String;");
    assert(methodId != NULL);

    jni::guard::string_utf8 stringValue = jni::string::get_chars_utf8(
        env, (jstring)env.instance().CallObjectMethod(value, methodId));

    // I don't think the JNI provides a method to figure out the length of a
    // string in ascii representation; it always just counts the amount of UTF-8
    // chars. As such we have to count the amount of bytes using strlen() here.
    jsize len = strlen(stringValue);

    qdb::jni::log::trace("Batch writer setting string at offset %d", index);
    return jni::exception::throw_if_error(
        handle,
        qdb_ts_batch_row_set_string(batchTable, index, stringValue, len));
}
                       ...
                       jobject value)
{
    ...
    case qdb_ts_column_int64:
        return tableRowSetInt64ColumnValue(env, handle, batchTable, index,
                                           value);
        break;

    case qdb_ts_column_double:
        return tableRowSetDoubleColumnValue(env, handle, batchTable, index,
                                            value);
        break;

    case qdb_ts_column_timestamp:
        return tableRowSetTimestampColumnValue(env, handle, batchTable, index,
                                               value);
        break;

    case qdb_ts_column_blob:
        return tableRowSetBlobColumnValue(env, handle, batchTable, index,
                                          value);
        break;

    case qdb_ts_column_string:
        return tableRowSetStringColumnValue(env, handle, batchTable, index,
                                            value);
        break;
               ...
               size_t count)
{
    ...
    jni::local_frame lf = jni::local_frame::push(
        env,
    ...
    jni::exception::throw_if_error(
        handle, qdb_ts_batch_start_row(batchTable, &nativeTime));

    for (size_t i = 0; i < count; i++)
    {
        jobject value = (jobject)(env.instance().GetObjectArrayElement(
            values, static_cast<jsize>(i)));
        jni::exception::throw_if_error(
            handle, tableRowSetColumnValue(env, handle, batchTable, (index + i),
                                           value));
    }
