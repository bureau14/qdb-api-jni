...
/* static */ qdb::jni::guard::string_utf8 qdb::jni::string::get_chars_utf8(
    qdb::jni::env & env, jstring str)
{
    assert(str != NULL);

    return qdb::jni::guard::string_utf8(env, str, env.instance().GetStringUTFChars(str, NULL),
        env.instance().GetStringUTFLength(str));
}

/* static */ qdb::jni::guard::local_ref<jstring> qdb::jni::string::create(
    jni::env & env, char const * str, jsize len)
{
    ...
    jni::env & env, char const * str, qdb_size_t len)
{
    ...
    return qdb::jni::guard::local_ref<jstring>(env, env.instance().NewStringUTF(copy));
}
