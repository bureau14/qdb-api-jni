...
Java_net_quasardb_qdb_jni_qdb_enable_1performance_1trace(JNIEnv *jniEnv,
                                                         jclass /* thisClass */,
                                                         jlong handle)
{
...
JNIEXPORT jint JNICALL
Java_net_quasardb_qdb_jni_qdb_disable_1performance_1trace(
...
{
    jni::guard::local_ref<jobject> output(
        jni::object::create(env, traceClass, traceConstructor,
    ...
    for (qdb_size_t i = 0; i < profile.count; ++i)
    {
        ...
        jni::guard::local_ref<jobject> measurement(
...
native_to_measurements(jni::env &env,
                       qdb_perf_profile_t const &profile,
                       jclass measurementClass,
                       jmethodID measurementConstructor)
{
...
JNIEXPORT jint JNICALL
Java_net_quasardb_qdb_jni_qdb_get_1performance_1traces(JNIEnv *jniEnv,
                                                       jclass /* thisClass */,
                                                       jlong handle,
                                                       jobject output)
{
    ...
    try
    {
        ...
        jni::exception::throw_if_error(
            (qdb_handle_t)handle,
            qdb_perf_get_profiles((qdb_handle_t)handle, &profiles,
                                  &profiles_count));

        //! Initialy class + functions once, cache them accross all operations
        jclass trace_class = jni::introspect::lookup_class(
            env, "net/quasardb/qdb/PerformanceTrace$Trace");

        jmethodID trace_constructor = jni::introspect::lookup_method(
            env, trace_class, "<init>",
            "(Ljava/lang/String;[Lnet/quasardb/qdb/"
            "PerformanceTrace$Measurement;)V");

        jclass measurement_class = jni::introspect::lookup_class(
            env, "net/quasardb/qdb/PerformanceTrace$Measurement");
        ...
        for (qdb_size_t i = 0; i < profiles_count; ++i)
        {
            env.instance().SetObjectArrayElement(
                xs, (jsize)i,
                jni::object::create(
                    env, trace_class, trace_constructor,
                    jni::string::create_utf8(env, profiles[i].name.data)
                        .release(),

                    native_to_measurements(env, profiles[i], measurement_class,
...
JNIEXPORT jint JNICALL
Java_net_quasardb_qdb_jni_qdb_clear_1performance_1traces(JNIEnv *jniEnv,
                                                         jclass /* thisClass */,
                                                         jlong handle)
{
