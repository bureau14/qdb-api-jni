...
nativeToValues(qdb::jni::env &env,
               qdb_point_result_t const values[],
               qdb_size_t count)
{
...
jni::guard::local_ref<jobjectArray>
nativeToColumnNames(qdb::jni::env &env,
                    qdb_string_t const columns[],
                    qdb_size_t count)
{

    jni::guard::local_ref<jobjectArray> output =
        jni::string::create_array(env, count);
...
nativeToResult(qdb::jni::env &env,
               qdb_query_result_t const &input,
               jclass resultClass)
{

    jclass rowClass =
        qdb::jni::introspect::lookup_class(env, "net/quasardb/qdb/ts/Row");
    ...
    {
        jni::guard::local_ref<jobject> row = jni::object::create(
            env, rowClass, "([Lnet/quasardb/qdb/ts/Value;)V",
...
Java_net_quasardb_qdb_jni_qdb_query_1execute(JNIEnv *jniEnv,
                                             jclass /*thisClass*/,
                                             jlong handle,
                                             jstring query,
                                             jobject outputReference)
{
    ...
    try
    {
        ...
        setReferenceValue(env, outputReference,
                          nativeToResult(env, *result,
                                         jni::introspect::lookup_class(
                                             env, "net/quasardb/qdb/ts/Result"))
                              .release());
